...
// ---- Constants ----
const WIND_DIR = 180; // Wind blows FROM the top of the screen (180 = from south in screen coords means arrow points down)
...
function windAngle(boatHeading: number): number {
  let diff = Math.abs(normalizeAngle(boatHeading) - normalizeAngle(WIND_DIR));
...
function getTackSide(boatHeading: number): 'port' | 'starboard' {
  const norm = normalizeAngle(boatHeading - WIND_DIR);
...
export default function SimulatorPage() {
  ...
  // Computed values
  const wa = windAngle(boatAngle);
  const pos = getPointOfSail(wa);
  const tack = getTackSide(boatAngle);
  ...
  useEffect(() => {
    ...
    const draw = (timestamp: number) => {
      ...
      const currentWA = windAngle(smoothAngleRef.current);
      ...
      const sectorStartRad = degToRad(WIND_DIR - NO_GO_HALF - 90);
      const sectorEndRad = degToRad(WIND_DIR + NO_GO_HALF - 90);
      ...
      for (const p of pointsOfSail) {
        ...
        for (const side of [-1, 1]) {
          const aStart = degToRad(WIND_DIR + p.angleMin * side - 90);
          const aEnd = degToRad(WIND_DIR + p.angleMax * side - 90);
          ctx.beginPath();
          ctx.moveTo(0, 0);
          if (side === 1) {
      ...
      const windArrowRad = degToRad(WIND_DIR - 90); // point from which wind comes
      ...
      const tackSign = getTackSide(smoothAngleRef.current) === 'port' ? 1 : -1;
      ...
      ctx.translate(cx, cy);
      const arcR = w * 0.18;
      const windScreenRad = degToRad(WIND_DIR - 90);
  ...
  // ---- Render ----
  return (
    <div className="page-enter flex flex-col lg:flex-row min-h-[calc(100vh-56px)]">
      {/* Canvas area */}
      <div
        ref={containerRef}
        ...
        <div className="card p-4">
          ...
          <div className="mb-3">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs" style={{ color: COLORS.textSecondary }}>Курс яхты</span>
