...
export default function SimulatorV3Page() {
  ...
  return (
    <div className="page-enter min-h-[calc(100vh-56px)] flex flex-col" style={{ background: '#050b18' }}>
      ...
      <footer className="px-3 sm:px-5 py-3 text-[10px] sm:text-xs text-[var(--text-muted)] leading-relaxed max-w-5xl mx-auto">
        {tp(
          'Сцена, метрики и комментарий читают один и тот же state. Контролы вокруг лодки - ничего не скролл-тить вниз чтобы что-то поменять.',
          'Scene, metrics, and commentary all read one state. Controls surround the boat - nothing to scroll down to.',
          'Scena i metryki dziela jeden stan.',
        )}
      </footer>
    </div>
  );
