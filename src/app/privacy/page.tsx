  ...
  return (
    <div className="page-enter max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      ...
      <div className="space-y-6 text-[var(--text-secondary)] leading-relaxed">
        {/* iOS app callout: the native app collects nothing. This MUST stay
            consistent with the App Store privacy disclosure (no tracking,
            no data collection) and with mobile/asc-metadata/PRIVACY_POLICY.md.
            The sections below describe the WEBSITE, which does use analytics. */}
        <div className="p-4 rounded-lg" style={{ background: 'rgba(68, 255, 136, 0.06)', border: '1px solid rgba(68, 255, 136, 0.25)' }}>
          ...
          <p className="text-sm">
            {tp(
              'Приложение «Week to Regatta» не собирает никаких данных, не отслеживает тебя и не передаёт ничего третьим лицам. Весь прогресс хранится локально на твоём устройстве (iOS AsyncStorage) и стирается в Настройки -> Данные -> Очистить. Сеть используется только когда ты сам запускаешь: галерея, AI-разбор гонки, баннер дневного челленджа - и идентификатор устройства при этом не отправляется. Разделы ниже описывают САЙТ regatta.icoffio.com, у которого аналитика есть.',
              'The "Week to Regatta" app collects no data, does not track you, and shares nothing with third parties. All progress is stored locally on your device (iOS AsyncStorage) and is wiped from Settings -> Data -> Clear all. The network is used only on actions you start: gallery, AI race review, daily-challenge banner - and no device identifier is attached. The sections below describe the WEBSITE regatta.icoffio.com, which does use analytics.',
              'Aplikacja "Week to Regatta" nie zbiera zadnych danych, nie sledzi cie i nie przekazuje nic stronom trzecim. Caly postep jest przechowywany lokalnie na urzadzeniu (iOS AsyncStorage) i kasowany w Ustawienia -> Dane -> Wyczysc. Siec dziala tylko gdy sam ja uruchomisz: galeria, analiza AI, baner wyzwania dnia - bez identyfikatora urzadzenia. Sekcje ponizej opisuja STRONE regatta.icoffio.com, ktora ma analityke.',
              {
                es: 'La aplicacion «Week to Regatta» no recopila datos, no te rastrea y no comparte nada con terceros. Todo el progreso se guarda localmente en tu dispositivo (iOS AsyncStorage) y se borra en Ajustes -> Datos -> Borrar todo. La red solo se usa en acciones que tu inicias: galeria, analisis de regata con IA, banner del reto diario, y no se adjunta ningun identificador del dispositivo. Las secciones siguientes describen el SITIO regatta.icoffio.com, que si usa analitica.',
                fr: 'L\'application « Week to Regatta » ne collecte aucune donnee, ne te suit pas et ne partage rien avec des tiers. Toute la progression est stockee localement sur ton appareil (iOS AsyncStorage) et effacee depuis Reglages -> Donnees -> Tout effacer. Le reseau n\'est utilise que pour les actions que tu lances : galerie, analyse de course IA, banniere du defi du jour, sans identifiant d\'appareil. Les sections ci-dessous decrivent le SITE regatta.icoffio.com, qui utilise de l\'analytique.',
                de: 'Die App «Week to Regatta» erfasst keine Daten, verfolgt dich nicht und teilt nichts mit Dritten. Der gesamte Fortschritt wird lokal auf deinem Geraet gespeichert (iOS AsyncStorage) und unter Einstellungen -> Daten -> Alles loeschen entfernt. Das Netzwerk wird nur bei Aktionen genutzt, die du startest: Galerie, KI-Rennanalyse, Tages-Challenge-Banner, ohne Geraetekennung. Die Abschnitte unten beschreiben die WEBSITE regatta.icoffio.com, die Analytik verwendet.',
                it: 'L\'app «Week to Regatta» non raccoglie dati, non ti traccia e non condivide nulla con terze parti. Tutti i progressi sono salvati localmente sul tuo dispositivo (iOS AsyncStorage) e cancellati da Impostazioni -> Dati -> Cancella tutto. La rete si usa solo per azioni che avvii tu: galleria, analisi gara con AI, banner della sfida del giorno, senza identificatore del dispositivo. Le sezioni sotto descrivono il SITO regatta.icoffio.com, che usa analitica.',
              },
            )}
          </p>
        </div>
        ...
        <h2 className="text-2xl font-bold pt-2 text-[var(--text-primary)]">
          {tp('Сайт regatta.icoffio.com', 'Website regatta.icoffio.com', 'Strona regatta.icoffio.com',
            { es: 'Sitio regatta.icoffio.com', fr: 'Site regatta.icoffio.com', de: 'Website regatta.icoffio.com', it: 'Sito regatta.icoffio.com' })}
        </h2>
