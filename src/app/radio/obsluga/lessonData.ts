...
export const LESSON_TEXT: Record<string, LessonText> = {
  ...
  distress: {
    ...
    examPl: 'Egzaminator patrzy na cala sekwencje: otwarcie oslony, przytrzymanie do konca odliczania, potem od razu MAYDAY glosem na 16 (ACK przychodzi w trakcie lub po nadaniu glosem) wedlug schematu MIPDANIO: Mayday (trzy razy), Identyfikacja (nazwa jachtu i MMSI), Pozycja, Danger (co sie stalo), Assistance (jakiej pomocy potrzebujesz), Number (ilu ludzi na pokladzie), Info (co jeszcze warto wiedziec: tratwa, kolor kadluba), Over. Klasyczny blad: za wczesne puszczenie klawisza (alert w ogole nie poszedl) i, jeszcze gorzej, niewycofanie falszywego alarmu - taki alarm anuluje sie przez DSC i glosem na 16, nigdy przez wylaczenie radia.',
    examRu: 'Экзаменатор смотрит на всю последовательность: открыть крышку, держать до конца отсчета, затем сразу голосом MAYDAY на 16 (ACK приходит во время или после голосовой передачи) по схеме MIPDANIO: Mayday (трижды), Identification (название яхты и MMSI), Position (позиция), Danger (что случилось), Assistance (какая помощь нужна), Number (сколько человек на борту), Info (что еще важно: плот, цвет корпуса), Over. Классическая ошибка: отпустить клавишу раньше времени (алерт вообще не ушел) и, что хуже, не отменить ложный алерт - его отменяют через DSC и голосом на 16, а не выключением рации.',
  },
};
