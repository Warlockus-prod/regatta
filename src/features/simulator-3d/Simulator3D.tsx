...
export function Simulator3D({ labels, headerSlot, className, initialMode = 'free' }: Simulator3DProps) {
  ...
  return (
    <div className={'mx-auto max-w-6xl px-4 py-4 ' + (className ?? '')}>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
        <div className="relative overflow-hidden rounded-xl border border-[rgba(255,255,255,0.08)]" style={{ height: '68vh', minHeight: 420 }}>
          <RegattaScene stateRef={yachtRef} />
            ...
            <>
              ...
              <div className="absolute right-3 top-3 flex flex-col items-end gap-1.5">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  ...
                  <Readout label={L.heel} value={`${t.heelDeg.toFixed(0)} deg`} />
                  <Readout label="VMG" value={`${t.vmg.toFixed(1)} kn`} />
                  <Readout label="TGT" value={`${t.targetSpeedKn.toFixed(1)} kn`} />
                </div>
                <div className="rounded bg-[rgba(4,22,30,0.7)] px-2 py-0.5 text-[10px] text-[var(--text-secondary,#9fb6c4)]">
                  {L.bestVmg}: {t.vmgTargetAngle.toFixed(0)} deg
                </div>
              </div>
              <div
        ...
        <div className="space-y-4 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.02)] p-4">
          {mode === 'free' ? (
            <>
              <div>
              ...
              <div className="space-y-3">
                <Slider label={L.mainsheet} value={free.boomAngle} min={0} max={85} step={1} onChange={(v) => setFreeField({ boomAngle: v })} fmt={(v) => `${v.toFixed(0)} deg`} />
                <Slider label={L.jibsheet} value={free.jibAngle} min={0} max={70} step={1} onChange={(v) => setFreeField({ jibAngle: v })} fmt={(v) => `${v.toFixed(0)} deg`} />
                <Slider label={L.camber} value={free.camber} min={0} max={1} onChange={(v) => setFreeField({ camber: v })} />
                <Slider label={L.twist} value={free.twist} min={0} max={1} onChange={(v) => setFreeField({ twist: v })} />
                <Slider label={L.luffing} value={free.luff} min={0} max={1} onChange={(v) => setFreeField({ luff: v })} />
                <Slider label={L.reef} value={free.reef} min={0} max={1} onChange={(v) => setFreeField({ reef: v })} />
                <Slider label={L.rudder} value={free.rudderAngle} min={-35} max={35} step={1} onChange={(v) => setFreeField({ rudderAngle: v })} fmt={(v) => `${v.toFixed(0)} deg`} />
                <Slider label={L.heel} value={free.heel} min={0} max={35} step={1} onChange={(v) => setFreeField({ heel: v })} fmt={(v) => `${v.toFixed(0)} deg`} />
              </div>
            ...
            <>
              <div className="space-y-3">
                <Slider label={L.helm} value={sim.controls.rudder} min={-1} max={1} onChange={(v) => sim.setControl('rudder', v)} fmt={(v) => (v === 0 ? '0' : `${(v * 35).toFixed(0)} deg`)} />
                ...
                <Slider label={L.mainsheet} value={sim.controls.mainSheet} min={0} max={1} onChange={(v) => sim.setControl('mainSheet', v)} />
                <Slider label={L.jibsheet} value={sim.controls.jibSheet} min={0} max={1} onChange={(v) => sim.setControl('jibSheet', v)} />
                <Slider label={L.reef} value={sim.controls.reef} min={0} max={1} onChange={(v) => sim.setControl('reef', v)} />
                <hr className="border-[rgba(255,255,255,0.08)]" />
                <Slider label={L.windSpeed} value={sim.wind.twsKn} min={4} max={28} step={1} onChange={(v) => sim.setWind((w) => ({ ...w, twsKn: v }))} fmt={(v) => `${v.toFixed(0)} kn`} />
                <Slider label={L.wind} value={sim.wind.fromDeg} min={0} max={359} step={1} onChange={(v) => sim.setWind((w) => ({ ...w, fromDeg: v }))} fmt={(v) => `${v.toFixed(0)} deg`} />
              </div>
