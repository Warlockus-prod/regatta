...
export function apparentWind(
  ...
  // Wind velocity vector in boat frame. Wind comes FROM twa, flows TO twa+180.
  // Unit vector toward twa+180 in (x=starboard, y=forward) axes:
  //   x = sin(twa + 180) = -sin(twa)
  //   y = cos(twa + 180) = -cos(twa)
  const windX = -tws * Math.sin(twaRad);
  const windY = -tws * Math.cos(twaRad);

  // Boat velocity in boat frame. With leeway, the boat track is slightly off
  // the bow axis. For wind calc this matters a little; keep it simple but
  // correct.
  const leewayRad = leewayDeg * DEG_TO_RAD;
  const boatX = bs * Math.sin(leewayRad);
  const boatY = bs * Math.cos(leewayRad);

  // Apparent wind = wind - boat velocity (air relative to boat).
  const awX = windX - boatX;
  const awY = windY - boatY;

  const awsMps = Math.hypot(awX, awY);

  // AWA = angle from bow to direction wind COMES FROM.
  // "From" direction vector = -AW_vec.
  // atan2(-awX, -awY) returns angle from +y axis, CW-positive = starboard-from.
