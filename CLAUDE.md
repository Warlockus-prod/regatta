...
- Tests: `npm run test:physics` = 31/31 green, `npx playwright test` = 12/12 green. Don't merge if red.
...
- Privacy/abuse hardening (2026-05-24): client IPs are truncated at ingest in `/api/log` (IPv4 last octet zeroed `1.2.3.0`, IPv6 kept to the first 3 hextets) before they are stored or logged, so `/stats` never surfaces full visitor IPs (header-based country detection is unaffected). `/api/log` persists only an allow-listed `meta` subset with a 4KB cap, not the raw body. `/api/race-result` is rate-limited (60/hr/session) with value bounds against leaderboard poisoning, and the `/stats` basic-auth uses a constant-time comparison.
